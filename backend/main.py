...
mongo = PyMongo(app)
db = mongo.cx['fitness-tracker-db']
...
def get_workouts(current_user):
  ...
  print(f"TYPE: {type}")
  workouts = db['workouts'].find({"user_id": current_user['_id'], "type": type})
...
def add_workout(current_user): 
  ...
  result = db['workouts'].insert_one(new_workout)
...
def delete_workout(current_user, id):
  result = db['workouts'].delete_one({"user_id": current_user['_id'], "_id": ObjectId(id)})
...
def update_workout(current_user, id):
  ...
  result = db['workouts'].update_one({"user_id": current_user["_id"], '_id': ObjectId(id)}, {'$set': updated_workout})
